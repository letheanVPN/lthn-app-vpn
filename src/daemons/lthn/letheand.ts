...
export class LetheanDaemonLetheand {
	...
	public static config() {
		...
		return new Command()
			...
			.option('--enforce-dns-checkpointing <boolean>', 'checkpoints from DNS server will be enforced')
			.option('--prep-blocks-threads <number>', 'Max number of threads to use when preparing block hashes in groups.')
			.option('--fast-block-sync <boolean>', 'Sync up most of the way by using embedded, known block hashes.')
			.option('--show-time-stats <boolean>', 'Show time-stats when processingblocks/txs and disk synchronization')
			...
			.option('--fluffy-blocks <boolean>', 'Relay blocks as fluffy blocks where possible (automatic on testnet)')
			.option('--standard-json <boolean>', 'Force standard JSON output (do not  return binary data in json fields)')
			...
			.option('--bg-mining-enable <boolean>', 'enable/disable background mining')
			.option('--bg-mining-ignore-battery <boolean>', 'if true, assumes plugged in when unable to query system power status')
			...
			.option('--db-salvage <boolean>', 'Try to salvage a blockchain database if it seems corrupted')
			...
			.option('--allow-local-ip <boolean>', 'Allow local ip add to peer list, mostly in debug purposes')
			...
			.option('--hide-my-port <boolean>', 'Do not announce yourself as peerlist candidate')
			.option('--no-igd <boolean>', 'Disable UPnP port mapping')
			.option('--offline <boolean>', 'Do not listen for peers, nor connect to any')
			.option('--out-peers <string>', 'set max number of out peers')
			.option('--tos-flag <boolean>', 'set TOS flag')

...
# This example is made for a Raspberry Pi 3B+ with Raspbian OS.
...
# with this script works with your specific setup.


EXTIF=eth0         # The name of the network interface which is connected to internet

POLICY=minimum     # See description of polices below
...
PROXYPORT2=        # This is the endport port set in sdp.json for the second proxy service
PROXYPORT3=        # This is the endport port set in sdp.json for the third proxy service

VPNPORT_UDP1=20001 # This is the endpoint port set in sdp.json for the first OpenVPN service if using UDP
VPNPORT_UDP2=      # This is the endpoint port set in sdp.json for the second OpenVPN service if using UDP
VPNPORT_UDP3=      # This is the endpoint port set in sdp.json for the third OpenVPN service if using UDP
...
DNS1=              # IP-address to DNS server that is forced to be used on VPN1 (clients on tun0 interface)
DNS2=              # IP-address to DNS server that is forced to be used on VPN2 (clients on tun1 interface)
DNS3=              # IP-address to DNS server that is forced to be used on VPN3 (clients on tun2 interface)

HOSTDNS=true       # Set to true if the exit node have a "bare metal" DNS server like unbound, dnsmasq or bind
...
PUBLICDNS=yes      # If set to yes it permits VPN clients to use unencrypted public DNS.
...
IPRANGE_TUN1=                # The IP address range used by tun1 interface. This is set in dispatcher.ini for second OpenVPN Service
IPRANGE_TUN2=                # The IP address range used by tun2 interface. This is set in dispatcher.ini for third OpenVPN Service
